...
class DistributedDataParallelConfig:
    ...
    """If true, reuse the grad buffer for param AG when params are kept in a quantized format.
       The distributed optimizer performs an out-of-place all-gather into the BF16 staging buffer
       (shared with the grad buffer), then copies BF16 into the quantized param (which triggers
       quantization internally). Generalizes over all quantized types (NVFP4, future FP2, etc.).
       Should be set to True only when fp4_param is True."""
    ...
    def __post_init__(self):
        ...
        if self.reuse_grad_buf_for_mxfp8_param_ag and self.reuse_grad_buf_for_high_precision_param_ag:
            raise ValueError(
                "reuse_grad_buf_for_mxfp8_param_ag and reuse_grad_buf_for_high_precision_param_ag "
